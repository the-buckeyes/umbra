use super::{
    algorithm::Algorithm, identity::Identity, organization::Organization, reply::Reply,
    system::System,
};
...
impl<'a> Registration<'a> {
    pub async fn into_identity(db: MySql, r: &Registration<'a>) -> Reply<Option<Identity>> {
        let (db, algo) = Algorithm::get_by_slug(db, "SCRYPT")
            .await
        ...
        let username_hash = get_username_hash(r.username, Some(algo), org, sys)?;

        Identity::insert(db, &username_hash, algorithm_id, None, &derived_key).await
    }
}
